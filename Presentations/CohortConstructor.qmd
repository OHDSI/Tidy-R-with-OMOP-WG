  ...
  revealjs: 
    ...
    footer: |
      <div style="position: relative; width: 100%;">
        <div style="text-align: center; font-weight: 500;">CohortConstructor R package</div>
...
-   *omopgenerics* and *CDMConnector* define an [ecosystem of R packages](https://ohdsi.github.io/Tidy-R-with-OMOP-WG/Presentations/ecosystem) that allow us to interact with an OMOP CDM instance using packages such as dplyr and tidyr.
...
library(dplyr)
cdm$person |>
  select(person_id, sex = gender_concept_id) |>
  left_join(
    cdm$observation_period |>
      select(person_id, obs_start = observation_period_start_date),
    by = "person_id"
  ) |>
  glimpse()
```
...
For more details on this approach please refer to the following book:

Burn, E., & Català, M. (2025). Tidy R programming with the OMOP Common Data Model (first edition). Zenodo. https://doi.org/10.5281/zenodo.17532124. [Online version](https://ohdsi.github.io/Tidy-R-programming-with-OMOP/)
...
```{r, echo=FALSE}
con <- DBI::dbConnect(drv = duckdb::duckdb(dbdir = "/Users/martics/Documents/delphi-100k_5.4_1.5.duckdb"))
...
```{r, echo=FALSE}
result <- summariseCohortAttrition(cohort = cdm$medications) 
result |> 
...
systBP
systBP <- list(syst_blood_presure = systBP$concept_id)
```

## Measurement based - Example {.incremental:FALSE}

2)  Create measurement based cohorts
...
```{r, echo=FALSE}
result <- summariseCohortAttrition(cohort = cdm$hyp_sbp) 
result |> 
  tableCohortAttrition(
    groupColumn = c("cohort_name"),
    hide = c("variable_level", "reason_id", "estimate_name", "cdm_name", settingsColumns(result))
...
```{r}
attr(cdm$hyp_sbp, "cohort_codelist")
```
...
summariseCohortCount(cohort = cdm$aspirin) |> 
  tableCohortCount()
```
...
-   *require* functions eliminate the records that **do not** satisfy the inclusion criteria at `indexDate`.
...
result <- cdm$medications_requirement |> 
  summariseCohortAttrition(cohortId = "acetaminophen") 
tableCohortAttrition(
  result = result,
  groupColumn = c("cohort_name"),
  hide = c("variable_level", "reason_id", "estimate_name", "cdm_name", settingsColumns(result))
...
-   We want to define a cohort of individuals taking `acetaminophen` and `ibuprofen` (prescriptions separated by 90 days or less are concatenated in a unique era of exposure) with at least 365 days of prior history, not taking opoiods on index date and censor individual if they start taking opioids.
...
<https://github.com/heron-uk/HERON-UK-02-003-CVDMedsInpatient/blob/main/study/Cohorts/InstantiateCohorts.R>
...
📧 <a href="mailto:nuria.mercadebesora@ndorms.ox.ac.uk">nuria.mercadebesora\@ndorms.ox.ac.uk</a> 📧 <a href="mailto:marti.catalasabate@ndorms.ox.ac.uk">marti.catalasabate\@ndorms.ox.ac.uk</a>
:::
