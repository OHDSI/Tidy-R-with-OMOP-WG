...
packages <- dplyr::tribble(
  ...
  "PatientProfiles", "Core", "darwin-eu", "Add columns related to patient demographics; Add columns related to intersections; Summarise data",
...
-   OMOP CDM the OMOP CDM is a fix structure that is known and all data is structured in the same way.
-   Complications to install packages, not in cran, java, contribute to the ecosystem
-   Tidyverse. Complete set of package for data manipulation: <https://tidyverse.org/>
...
The presented ecosystem sits on top of these functions combined with the known structure of the [OMOP CDM](https://ohdsi.github.io/CommonDataModel/) tables to build packages to perform Epidemiological studies extracting data from an OMOP CDM source that can be in several different back-ends.
  ...
  purrr::pmap(\(package_name, classification, last_version, title, description, website, github, n_releases, first_release, key_functions, logo) {
    c(
      ...
      if_else(is.na(logo), "", paste0(
        '\n<div style="position:absolute; top:50px; right:20px;"><img src="', logo, '" width="100"></div>\n'
      )),
      ". . .",
      "",
      paste0("**", title, "**"),
...
- The CDM reference object: `cdm_reference`, `cdm_source`, `local_source`
...
For example for CDMConnector, the source contains the `connection` and the `writeSchema`:
...
Then later can also be imported:
...
The `summarised_result` is the result model defined by *omopgenerics*. It contains 13 compulsory columns and it has its own methods.
...
## Attributes of `summarised_result`
...
```{r, message=TRUE}
isResultSuppressed(result)
```

## `summarised_result`

```{r, message=TRUE}
result <- suppress(result = result, minCellCount = 10)
isResultSuppressed(result)
```

## Creating a `summarised_result`

You can create a `summarised_result` object with `newSummarisedResult()`:

```{r}
sr <- newSummarisedResult(
  x = tibble(
    result_id = 1L,
    cdm_name = "My OMOP Instance",
    group_name = "cohort_name",
    group_level = "acetaminophen",
    strata_name = "sex",
    strata_level = "Female",
    variable_name = c("Nuber subjects", rep("Age", 3)),
    variable_level = NA_character_,
    estimate_name = c("count", "median", "q25", "q75"),
    estimate_type = "integer",
    estimate_value = c("100", "40", "25", "58"),
    additional_name = "overall",
    additional_level = "overall"
  ),
  settings = tibble(
    result_id = 1L,
    result_type = "characterisation",
...
You can split the 'group', 'strata' or 'additional' aggregations using:
...
You can filter a `summarised_result` object using:
...
The inverse of "tidy" can also be done and this is how usually the `summarised_result` object is created.

```{r}
result <- result |> filterSettings(result_type == "incidence") |> tidy()
class(result)
result
```
...
```{r}
sr_result <- transformToSummarisedResult(
  x = result,
  group = c("denominator_cohort_name", "outcome_cohort_name"),
  additional = c("incidence_start_date", "incidence_end_date", "analysis_interval"),
  estimates = c("denominator_count", "outcome_count", "person_days", "person_years", "incidence_100000_pys", "incidence_100000_pys_95CI_lower", "incidence_100000_pys_95CI_upper"),
  settings = c("analysis_censor_cohort_name", "analysis_complete_database_intervals", "analysis_outcome_washout", "analysis_repeated_events", "denominator_age_group", "denominator_days_prior_observation", "denominator_end_date", "denominator_requirements_at_entry", "denominator_sex", "denominator_start_date", "denominator_target_cohort_name", "denominator_time_at_risk")
)
sr_result
```
...
You can also use [PatientProfiles](https://darwin-eu.github.io/PatientProfiles/) to summarise data from a dataset into summarised result:
...
result <- summariseResult(
  ...
  estimates = list(
    c("mean", "sd", "percentage_missing"),
    c("count", "percentage", "percentage_missing"),
    c("median", "q25", "q75", "percentage_missing")
  )
)
```
...
- The **omopgenerics** website: <https://darwin-eu.github.io/omopgenerics/>
- Reexporting from *omopgenerics*: <https://darwin-eu.github.io/omopgenerics/articles/reexport.html>
- Expanding *omopgenerics*: <https://darwin-eu.github.io/omopgenerics/articles/expanding_omopgenerics.html>
- Tidy R packages foundations: <https://ohdsi.github.io/Tidy-R-programming-with-OMOP/>
- Tidy R packages development: <https://oxford-pharmacoepi.github.io/Tidy-R-packages-with-OMOP/>
